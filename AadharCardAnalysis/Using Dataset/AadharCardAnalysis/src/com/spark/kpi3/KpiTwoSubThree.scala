...
import org.apache.spark.sql.functions.{ row_number, when, count, desc, sum, col }
import org.apache.spark.sql.types._

object KpiTwoSubThree {
  ...
  def main(args: Array[String]) {
    ...
    // Mobile Count
    val mobisCount = data.agg(count(when(col("_c11").>(0), 1)).alias("Mobie Count")).show()

    // Email Count
    val emailCount = data.agg(count(when(col("_c12").>(0), 1)).alias("Email Count")).show()
