...
object KpiThreeSubOne {
  ...
  def main(args: Array[String]) {
    ...
    // Total count of people who provided mobile Number
    val count = data.agg(sum("_c11")).first().get(0)
       
    val result = data.groupBy("_c8")
      .agg((sum("_c11") * 100.0 / count).alias("mobile_provided_percentage"))
      .show(100)
