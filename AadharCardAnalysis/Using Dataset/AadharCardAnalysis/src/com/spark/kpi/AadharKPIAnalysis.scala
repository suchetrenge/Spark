...
object AadharKPIAnalysis {
  ...
  def main(args: Array[String]) {
    ...
    val windowFunc = Window.partitionBy("_c2").orderBy(desc("_c9"))
    
    val result = data.withColumn("row", row_number.over(windowFunc))
    .where($"row" <= 25).drop("row")
