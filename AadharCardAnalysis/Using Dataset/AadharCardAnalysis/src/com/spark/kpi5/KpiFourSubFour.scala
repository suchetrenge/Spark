...
object KpiFourSubFour {
  ...
  def main(args: Array[String]) {
    ...
    val count = data.where(col("_c7").===("F")).agg(sum("_c9")).first().get(0)

    // Total count of aadhar rejected for Males
    val countMale = data.where(col("_c7").===("M")).agg(sum("_c10")).first().get(0)

    val result = data.filter(col("_c7").===("F"))
      .groupBy("_c3")
    ...
    val result2 = data.filter(col("_c7").===("M") && (col("_c3").===(statelist(0)) || col("_c3").===(statelist(1)) || col("_c3").===(statelist(2))))
      .groupBy("_c3")
      .agg((sum("_c10") * 100.0 / countMale).alias("most_aadhar_rejected"))
