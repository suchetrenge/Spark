package com.spark.kpi5

import org.apache.spark.sql.SparkSession
import org.apache.spark.sql.expressions.Window
import org.apache.spark.sql.functions.{ row_number, when, max, count, desc, sum, col }
import org.apache.spark.sql.types._

object KpiFourSubTwo {
  case class aadharCase(
    _c0:  String,
    _c1:  String,
    _c2:  String,
    _c3:  String,
    _c4:  String,
    _c5:  String,
    _c6:  String,
    _c7:  String,
    _c8:  String,
    _c9:  String,
    _c10: String,
    _c11: String,
    _c12: String);

  def main(args: Array[String]) {
    System.setProperty("hadoop.home.dir", "D:\\Study\\DataFlair\\software\\hadoop\\hadoop-3.1.2")
    System.setProperty("spark.sql.warehouse.dir", "file:///D:/Study/DataFlair/software/spark/spark-2.0.2-bin-hadoop2.6/spark-warehouse")

    //    if (args.length < 2) {
    //      System.err.println("Usage: JavaWordCount <Input-File> <Output-File>");
    //      System.exit(1);
    //    }

    val spark = SparkSession.builder().appName("Spark SQL").master("local").getOrCreate()

    import spark.implicits._
    val path = "D:\\Study\\DataFlair\\Assignement_&_Projects\\projects_spark\\Aadhar_Card_Analysis\\data\\data\\aadhaar\\aadhaardata.csv"
    val data = spark.read.csv(path).as[aadharCase]

    // Total count of aadhar generated for Males
    val count = data.where(col("_c7").===("M")).agg(sum("_c9")).first().get(0)

    // Total count of aadhar rejected for Females
    val countFemale = data.where(col("_c7").===("F")).agg(sum("_c10")).first().get(0)

    val result = data.where(col("_c7").===("M"))
      .groupBy("_c3")
      .agg((sum("_c9") * 100.0 / count).alias("most_aadhar_generated"))
      .orderBy(desc("most_aadhar_generated"))
      .select("_c3")
      .collect()
      .map(_(0))
      .toList

    val statelist = result.take(3)

    val result2 = data.filter(col("_c7").===("F") && (col("_c3").===(statelist(0)) || col("_c3").===(statelist(1)) || col("_c3").===(statelist(2))))
      .groupBy("_c3")
      .agg((sum("_c10") * 100.0 / countFemale).alias("most_aadhar_rejected"))
      .orderBy(desc("most_aadhar_rejected"))
      .show(3)

    spark.stop
  }

}